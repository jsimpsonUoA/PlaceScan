...
def save_data(filename, picks=None, early_err_=None, late_err_=None, update_picks=True,
    ...
    picks_dicts = [pick_times, early_err, late_err]
    if update_picks:
        prev_picks_dicts = picks_data_from_csv(filename)
        for i in range(3):
            prev_picks_dicts[i].update(picks_dicts[i])
        picks_dicts = prev_picks_dicts

    for i, times_dict in enumerate(picks_dicts):
        for key, val in times_dict.items():
            times_dict[key] = val + [arrival_time_correction, early_err_corr+arrival_time_correction,\
                                                     late_err_corr+arrival_time_correction][i]  
    items = [list(picks_dicts[0].items()), list(picks_dicts[1].items()), list(picks_dicts[2].items())]
...
def correct_picks(filename, pick_corr=0.0, early_err=0.0, late_err=0.0):
    '''
    A function to apply a correction to the  picks in a
    picks csv file. The early_err and late_err will become
    the new early and late errors, so must be specified, even
    if they do not change from the previous version
    '''
    
    data = picks_data_from_csv(filename)
    
    save_data(filename, picks=data[0], early_err_=None, late_err_=None, update_picks=False,
         arrival_time_correction=pick_corr, early_err_corr=early_err, late_err_corr=late_err)


def assign_percentage_errors(filename, percentage, constant_early_err=0.0, constant_late_err=0.0):
    '''
    A function to make the error of each pick plus or minus
    ...
    data = picks_data_from_csv(filename)
    x_pos = np.array(list(data[0].keys()))
    picks = np.array(list(data[0].values()))

    percentages = percentage / 100. * picks
    indices = np.where(percentages < 0.)

    early_err = -1. * percentages + constant_early_err
    late_err = percentages + constant_late_err
    ...
    early_err = dict(zip(x_pos,early_err+picks))
    late_err = dict(zip(x_pos,late_err+picks))

    save_data(filename, picks=data[0], early_err_=early_err, late_err_=late_err, update_picks=False,
         arrival_time_correction=0., early_err_corr=0., late_err_corr=0.)
...
class AICPicker():
    ...
    def plot_aic_pick(self, position):
        ...
        fig, ax[0] = format_fig(fig, ax[0],ylab='$u_i(t)$',show=False)
        fig, ax[1] = format_fig(fig, ax[1],xlab='Time ($\mu$s)',ylab='AIC',show=True,save_dir=None)#'aic_example_7f-schist-2-30deg.pdf')
