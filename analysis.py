...
class RPCalculator():
    def __init__(self, sample_name, picks_dirs, rp_save_filename,
                labels, data_filename, s_wave_picks_dir='', 
                picks_offset=0., picks_xlim=None):
        '''
        ...
        self.s_wave_picks = self.retrieve_sample_data(s_wave_picks_dir, raise_exception=False)
        self.max_ind, self.min_ind = self.get_max_min_inds()  #Indices of max and min arrival times

        self.data.update(self.sample_data)
        self.data.update(self.s_wave_picks)
        self.save_rp_data()
    ...
    def get_max_min_inds(self):
        ...
        Returns:
            --max_ind, min_ind: The indices in the scans of the max and min arrival times.
        '''
        ...
        self.data['max_ind'] = max_ind
        self.data['min_ind'] = min_ind
    ...
    def calculate_anisotrpy(self):
        ...
        for i, headers, x_pos, times in self.picks_iterator():
            if headers:  
                v_max, v_min = 1/times[self.min_ind], 1/times[self.max_ind]
    ...
    def calculate_cijs(self):
        ...
        for i, headers, x_pos, times in self.picks_iterator():
            ...
            if headers:  
                v_max, v_min = diameter/times[self.min_ind], diameter/times[self.max_ind]
                cij_dict['c11'] = v_max**2 * density
                cij_dict['c33'] = v_min**2 * density

            try:
                s0 = diameter/self.s_wave_picks[self.labels[i]]['s0']
                cij_dict['c55'] = s0**2 * density
            except KeyError:
                pass

            try:
                sh90 = diameter/self.s_wave_picks[self.labels[i]]['sh90']
...
def construct_latex_table(master_dir, samples, labels, variables,
    ...
    for i, keys in enumerate(indexing_list):
        ...
        if str(value.n) != str(np.nan):
            values.append(r'{:L}'.format(value))
        else:
            values.append(r'-')
